...
const MeasurementForm: React.FC<MeasurementFormProps> = ({ model, onBack, onSubmit }) => {
  ...
  const handleInputChange = (id: string, value: string) => {
    // Se for SHOE_SIZE, garantimos que seja um número inteiro
    if (id === 'SHOE_SIZE') {
    ...
    <div className="w-full max-w-6xl mx-auto px-4">
      ...
      <div className="grid md:grid-cols-2 gap-12">
        <form onSubmit={handleSubmit}>
          {model.requiredMeasurements
            .filter(id => !model.specialInputs?.includes(id)) // Não mostrar medidas que são specialInputs
            .map(id => (
              <InputField
                key={id}
                id={id}
